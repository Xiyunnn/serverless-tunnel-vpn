import { connect } from "cloudflare:sockets";
    ...
    // Variables
    const rootDomain = "naycilla.dpdns.org"; // Ganti dengan domain utama kalian
    const serviceName = "thefool-vpn"; // Ganti dengan nama workers kalian
    const apiKey = "63220db60f93a20823e5395b3d7c1bcfa4814"; // Ganti dengan Global API key kalian (https://dash.cloudflare.com/profile/api-tokens)
    const apiEmail = "usrpremium4@gmail.com"; // Ganti dengan email yang kalian gunakan
    const accountID = "b4dda49bce84c6715bcc7244dbef37d5"; // Ganti dengan Account ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
    const zoneID = "689a3fd65d17d7e6897d6fec66ede2a8"; // Ganti dengan Zone ID kalian (https://dash.cloudflare.com -> Klik domain yang kalian gunakan)
    ...
    const KV_PROXY_URL = "https://raw.githubusercontent.com/Xiyunnn/serverless-tunnel-vpn/refs/heads/main/kvProxyList.json";
    ...
    const DONATE_LINK = "https://saweria.co/Xiyunn";
    ...
    try {
        ...
        document.setTitle(
          "Welcome to <span class='text-blue-500 font-extrabold drop-shadow-lg animate-fade-in' style='animation-duration:1s;animation-delay:0.1s;display:inline-block;font-size:2.5rem;font-family:Montserrat,Inter,sans-serif;'>naycilla-project</span>"
        );

        // Info styling - gunakan font besar, bold, dan spacing
        document.addInfo(`
          <span class="block text-lg sm:text-xl font-bold tracking-wide text-blue-700 dark:text-blue-300 font-montserrat">
            Total Proxy: <span class="font-mono text-gray-900 dark:text-gray-100">${proxyList.length}</span>
          </span>
        `);
        const totalPages = Math.max(1, Math.ceil(proxyList.length / PROXY_PER_PAGE));
        document.addInfo(`
          <span class="block text-lg sm:text-xl font-bold tracking-wide text-purple-700 dark:text-purple-300 font-montserrat">
            Page: <span class="font-mono text-gray-900 dark:text-gray-100">${page + 1}</span>
            <span class="mx-1 text-gray-400 dark:text-gray-600">/</span>
            <span class="font-mono text-gray-900 dark:text-gray-100">${totalPages}</span>
          </span>
        `);

        // Info waktu, IP, dan ISP dengan styling besar dan font berbeda
        const now = new Date();
        const pad = n => n.toString().padStart(2, "0");
        const timeString = `${pad(now.getDate())}-${pad(now.getMonth() + 1)}-${pad(now.getFullYear())} ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`;
        const ip =
        ...
        const isp =
            ...
            (request.cf && request.cf.asn && "AS" + request.cf.asn) ||
            "-";
        document.addInfo(`
          <span class="block text-base sm:text-lg font-semibold text-green-700 dark:text-green-300 font-montserrat">
            Time: <span class="font-mono text-gray-900 dark:text-gray-100 text-lg" id="realtime-clock">${timeString}</span>
          </span>
        `);
        document.addInfo(`
          <span class="block text-base sm:text-lg font-semibold text-orange-700 dark:text-orange-300 font-montserrat">
            Your IP: <span class="font-mono text-gray-900 dark:text-gray-100 text-lg">${ip}</span>
          </span>
        `);
        document.addInfo(`
          <span class="block text-base sm:text-lg font-semibold text-pink-700 dark:text-pink-300 font-montserrat">
            ISP: <span class="font-mono text-gray-900 dark:text-gray-100 text-lg">${isp}</span>
          </span>
        `);
    ...
    <!-- Main Container -->
    <div class="container mx-auto px-2 sm:px-4 py-4 sm:py-8 max-w-7xl" style="margin-left:5.5rem">
        <!-- Header -->
        <header class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-2 sm:gap-0">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white gradient-text bg-gradient-to-r from-blue-500 to-purple-600 animate-fade-in" style="animation-duration:1s;animation-delay:0.1s;">
            PLACEHOLDER_JUDUL
            </h1>
        ...
        <!-- Proxy Grid -->
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mb-8 sm:mb-12">
        PLACEHOLDER_PROXY_GROUP
        </div>

        <!-- Pagination -->
        <div id="container-pagination" class="fixed bottom-4 sm:bottom-6 left-0 right-0 flex justify-center transition-transform duration-300 translate-y-0">
        <div class="bg-white dark:bg-dark-800 rounded-full shadow-lg px-3 sm:px-4 py-1.5 sm:py-2 flex items-center gap-2 border border-gray-200 dark:border-gray-700">
    ...
    <div id="container-window" class="fixed inset-0 bg-black bg-opacity-50 z-40 hidden transition-opacity duration-300 flex items-center justify-center">
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            klik ini untuk copy akun v2ray
            </button>
        </div>
    ...
    <script>
        ...
        const url = "https://naycilla.dpdns.org/api/v1/domains/get";
        ...
        const domain = rawDomain + ".naycilla.dpdns.org";
        // Cegah double root domain
        if (domain.endsWith(".naycilla.dpdns.org.naycilla.dpdns.org")) {
        ...
        const url = "https://naycilla.dpdns.org/api/v1/domains/put?domain=" + domain;
    ...
    // Tambahkan font Montserrat ke baseHTML (di <head>)
